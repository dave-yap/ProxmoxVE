...
echo -e "\nALLOWED_HOSTS = ['$IP']" >> /opt/seafile/conf/seahub_settings.py
echo -e "CSRF_TRUSTED_ORIGINS = ['http://$IP:8000']" >> /opt/seafile/conf/seahub_settings.py
echo -e "CSRF_COOKIE_SECURE = False" >> /opt/seafile/conf/seahub_settings.py
...
sed -i "s|CSRF_COOKIE_SECURE = False|CSRF_COOKIE_SECURE = True|g" /opt/seafile/conf/seahub_settings.py
EOF
chmod +x ~/domain.sh
