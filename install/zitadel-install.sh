...
tar -xzf /opt/${APP}/zitadel-linux-$ARCH.tar.gz
sudo mv /opt/${APP}/zitadel-linux-$ARCH/zitadel /usr/local/bin
...
User=zitadel
Group=zitadel
ExecStart=/usr/local/bin/zitadel start --masterkey "$(cat /opt/zitadel/.masterkey)" --tlsMode "$(cat /opt/zitadel/.tlsmode)" --config "$(cat /opt/zitadle/.config)"
Restart=always
RestartSec=5
TimeoutStartSec=0
