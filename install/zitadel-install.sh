...
wget -q -c https://github.com/zitadel/zitadel/releases/download/$LATEST/zitadel-linux-$ARCH.tar.gz -O - &>/dev/null | tar -xz &>/dev/null
...
timeout --kill-after=5s 30s zitadel start-from-init --masterkeyFile /opt/zitadel/.masterkey --config /opt/zitadel/config.yaml &>/dev/null
useradd zitadel
